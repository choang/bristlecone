...
public interface SqlDialect
{
  ...
  /**
   * Returns true if this is the right implementation to use for the given JDBC
   * URL.
   */
  ...
  /**
   * Returns an UPDATE statement that updates a all records where key column value is like value
   * given as second statement parameter
   */
  ...
  /**
   * Returns a SELECT statement to fetch a row using its primary key, which must
   * be supplied as a prepared statement parameter.
   */
  ...
  /**
   * Returns a SELECT statement to fetch one or more rows with a LIMIT. Not all
   * DBMS implementations can handle a limit.
   */
